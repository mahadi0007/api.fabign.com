...
class AdditionalinfoController {
    ...
    async addAdditionalInfo(req, res) {
        try {
            ...
            const isExist = await AdditionalInfo.findOne({ product: mongoose.Types.ObjectId(product) })
            if (isExist) {
                return failure(res, "Additional info is already available.", {})
            } else {
                let info = await new AdditionalInfo(req.body)
                info = await info.save();
                await Product.updateOne({ _id: mongoose.Types.ObjectId(product) }, {
                    $set: { additionalInfo: true }
                })
                return info ? success(res, "Additional Info Created", info) : failure(res, "Failed to create additional info", {})
            }
    ...
    async editAdditionInfo(req, res) {
        try {
            ...
            const modifyInfo = await AdditionalInfo.updateOne({ _id: mongoose.Types.ObjectId(id) }, {
                $set: req.body
            })

            return modifyInfo ? success(res, "Additional Info successfully modified", modifyInfo) :
    ...
    async deleteAdditionalInfo(req, res) {
        try {
            const info = await AdditionalInfo.findOne({ _id: mongoose.Types.ObjectId(req.params.id) })
            await Product.updateOne({ _id: mongoose.Types.ObjectId(info.product) }, {
            ...
            return deleted ? success(res, "Successfully Deleted", deleted) : notModified(res, "Not deleted", {})
        } catch (error) {
            return failure(res, error.message, error);
        }
    }
    ...
    async getAllAdditionalInfo(req, res) {
        try {
            ...
            return info ? success(res, "Additional info Fetched", {
                page: page,
                limit: limit,
                total: total,
                info
    ...
    async getProductWise(req, res) {
        try {
            const info = await AdditionalInfo.findOne({ product: mongoose.Types.ObjectId(req.params.id) })
                .populate({
                    path: "product",
                    select: "name _id"
                })
    ...
    async getSingleAdditionalInfo(req, res) {
        try {
            ...
            return info ? success(res, "AdditionalInfo Fetched", {
                info
            }) : notFound(res, "No content Found", {
                info: {}
            })
    ...
    async getProductwithAdditionalInfo(req, res) {
        try {
            ...
            //     $set: { additionalInfo: false }
            // })
            const productWithoutFAQ = await Product.find({ additionalInfo: false }, { name: 1 })

            return productWithoutFAQ ? success(res, "Product Fetch that don't have any faq", productWithoutFAQ) : failure("No Product exist")
        } catch (error) {
