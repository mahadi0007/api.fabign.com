...
const Store = data => {
    ...
    if (!data.sub_category || isEmpty(data.sub_category)) error.sub_category = "Sub category is required"
    ...
    if (data.files) {
        if (!data.files.title_image || isEmpty(data.files.title_image)) error.title_image = "Title image is required"
